...
\title{%
  ...
  \large \normalfont A Minimal Spectral Latent Diffusion Model}
}
...
\author{%
  Lorenzo Moriondo \\[2pt]   \small tuned.org.uk\\   \small ORCID: 0000-0002-8804-2963\\   \texttt{tunedconsulting@gmail.com} }  \date{July 2026 \\
    \href{https://github.com/tuned-org-uk/arrowspace-latent-diffusion}{%
     ...
     \emph{Predecessor (full entropic clock):}
    ...
    \begin{framed}
    \noindent\textbf{Codebase clarification.} This paper presents
    \ALDSC, a \emph{minimal} spectral latent diffusion model. The
    full entropic-clock formulation --- wave recurrence, vibrational
    pump, density matrices, and the bright--dark KL clock --- is
    implemented in the predecessor repository
    \texttt{entropic-semantic-diffusion} and is cited here as
    background. The code at \texttt{arrowspace-latent-diffusion}
    implements only the minimal viable architecture described in
    Sections~\ref{sec:25d}--\ref{sec:objective} and
    Section~\ref{sec:experiments}. Section~\ref{sec:entropic_clock}
    (entropic time) is a \emph{theoretical extension} with a thin
    reproducible stub (Section~\ref{sec:per_mode_schedule}), not a
    claim about the current implementation.
    \end{framed}

    \begin{abstract}
    We present \ALDSC~(\emph{Arrowspace Latent Diffusion with Spectral
    Chart Conditioning}), a minimal spectrally conditioned latent
    diffusion model that builds on the theoretical framework of the
    Entropic Semantic Diffusion Model (\ESDM)~\cite{Moriondo2026ESDM_code}
    while deferring its full entropic-clock machinery.

    The model encodes each image into a 2.5-D representation: a
    standard spatial VAE latent $z$ for local detail, and a feature
    field $A$ projected onto the smooth eigenspace of a \emph{frozen}
    ArrowSpace graph Laplacian. From this projection we extract a
    compact spectral chart $c_{\mathrm{spec}}$ of dimension $3q$,
    which conditions both a DiT denoiser (via adaptive layer
    normalisation) and a topology-adaptive decoder (via spectral
    gates). Diffusion operates only on $z$; the spectral chart is a
    side-channel, not a second diffused state.

    We implement the full pipeline --- prior construction, spectral
    VAE, v-prediction diffusion training, and DDIM/Euler sampling ---
    in under 700 lines of PyTorch, validated by 67 unit tests on CPU.
    The model generates images end-to-end via a single CLI command.

    The central falsifiable claim is that a frozen ArrowSpace spectral
    chart improves latent-image decoding and control at fixed latent
    capacity. The entropic clock (per-mode $\tau_k(t) = \nu_k \cdot t$)
    is included as a theoretical extension with a reproducible stub,
    positioning the upgrade path from \ALDSC~back to the full \ESDM.
    \end{abstract}
    ...
    The entropic clock is revisited as a theoretical extension in
    Section~\ref{sec:entropic_clock}, with a thin reproducible stub
    demonstrating the per-mode schedule formula on frozen eigenvalues.
    ...
    \noindent\textbf{Implementation:} \texttt{arrow_prior.py} --
    ...
    \subsection{Topology-adaptive decoder}
    \label{sec:decoder}
    The decoder reconstructs pixels from the denoised latent under
    spectral control: $\hat x = D_\psi(\hat z; c_{\mathrm{spec}})$.

    The design target is a multi-resolution decoder where each stage
    projects feature activations onto the chart basis, gates them by
    $c_{\mathrm{spec}}$, and lifts back:
    \[H^{(\ell+1)} = \mathrm{UpBlock}\!\left( H^{(\ell)} +
    (\hat H^{(\ell)} \odot g^{(\ell)})\Uq^\top \right),\]
    where $\hat H^{(\ell)} = H^{(\ell)}\Uq$ and $g^{(\ell)} =
    \sigma(W_\ell c_{\mathrm{spec}} + b_\ell)$.

    \noindent\textbf{Implementation:} The current \texttt{vae.py}
    implements a \emph{simplified} version of this formula: a single
    spectral gate at the top decoder level, $h \leftarrow h \cdot
    (1 + \sigma(W c_{\mathrm{spec}}))$, followed by standard residual
    up-blocks. The multi-resolution per-stage gating formula above is
    the design target for future versions. This simplification is
    tracked as issue \#4 in the repository.

    This construction makes the decoder topology-adaptive:
    reconstruction is not performed relative to an isotropic ambient
    prior alone but relative to the smooth directions described by the
    ArrowSpace graph.
    ...
    \begin{framed}
    \noindent\textbf{Theoretical extension.} The material in this
    section is \emph{not} part of the current \ALDSC~implementation.
    It describes the upgrade path from \ALDSC~back to the full \ESDM.
    A thin reproducible stub (Section~\ref{sec:per_mode_schedule})
    demonstrates the per-mode formula on frozen eigenvalues.
    \end{framed}
    ...
    % ------------------------------------------------------------
    \subsection{Per-mode entropic schedule (reproducible stub)}
    ...
    criterion $\sum_k \nu_k \cdot \bar\alpha_k(t) < \varepsilon$. All
    components are frozen (zero \texttt{nn.Parameter}). 16 unit tests
    verify shapes, monotonicity, boundary cases, and the $\nu_k$
    cancellation.
    ...
    Per-mode schedule & Spectral variant & No & Yes (theoretical) \\
    Geometry source & Image frequency & Image spectrum & Feature-space graph \\
    Online adaptation & No (post-train) & No & Yes (MMSE, future) \\
    ...
    \item \textbf{Spectral schedule stub:} the per-mode $\tau_k(t)$,
        $\bar\alpha_k$, entropy rate, and heat-death metric are
        computed correctly on frozen $\nu_k$.
    \end{itemize}
    ...
    \item Experiments use synthetic noise data, not real images. Real
        ...
        the next step.
    \item The topology decoder uses a simplified single-level spectral
        gate, not the full multi-resolution UpBlock formula.
    \item The entropic clock is a stub, not an active training
        schedule.
    ...
    and how their energy is distributed. The spectral schedule stub
    (Section~\ref{sec:per_mode_schedule}) provides the minimal
    reproducible bridge.
    ...
    a theoretical extension with a reproducible stub, positioning the
    upgrade path to the full \ESDM.
    ...
    \S\ref{sec:decoder}: topology decoder & \texttt{vae.py} (simplified) & Partial \\
    \S\ref{sec:objective}: losses & \texttt{losses.py} & Implemented \\
    \S\ref{sec:heat_kernel}: heat-kernel prior & --- & Theoretical \\
    \S\ref{sec:entropic_clock}: entropic clock & \texttt{spectral\_schedule.py} & Stub \\
    \S\ref{sec:per_mode_schedule}: per-mode schedule & \texttt{spectral\_schedule.py} & Stub \\
    \S\ref{sec:heat_death_clock}: heat death & \texttt{spectral\_schedule.py} & Stub \\
