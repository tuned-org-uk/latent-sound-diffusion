...
\author{%
  Lorenzo Moriondo \\[2pt]   \small tuned.org.uk\\   \small ORCID: 0000-0002-8804-2963\\   \texttt{tunedconsulting@gmail.com} }  \date{July 2026 \\
    ...
    (entropic time) is an active part of the inference pipeline
    (Section~\ref{sec:per_mode_schedule}): the heat-death stopping
    criterion terminates sampling when the spectrally-weighted metric
    ...
    is an active part of the inference pipeline: the sampler terminates
    when the spectrally-weighted heat-death metric drops below a threshold,
    rather than at a fixed step count. The Barontini principle survives
    but in a narrower form than a per-mode noise schedule --- it governs
    *when* the process terminates, not *how much* each mode is noised.
    \end{abstract}
