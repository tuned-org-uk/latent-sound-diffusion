"""Tests for clock-gated decoding tempo.

The SpectralSchedule governs when each spectral mode resolves during
decoding. High-ν_k modes (local detail) resolve at early decoder stages;
low-ν_k modes (global structure) resolve throughout. The decoder can
also use the heat-death metric to decide when reconstruction is complete.
"""
...
from ald_sc.graph_decoder import ClockGatedGraphDecoder
...
class TestClockGatedGraphDecoder:
    ...
    def test_clock_gates_mode_resolution(self) -> None:
        """The clock should modulate how strongly each wave block activates
        based on the spectral schedule. At different diffusion times,
        the gate strengths should differ."""
        prior, sched = _make_schedule()
        decoder = ClockGatedGraphDecoder(
            latent_channels=4,
            out_channels=3,
        ...
        # Early in the process (t near 1.0 = high noise), gates should be weaker
        # Late in the process (t near 0.0 = low noise), gates should be stronger
    ...
    def test_uses_wave_blocks(self) -> None:
        ...
        from ald_sc.graph_decoder import WaveReconstructionBlock

        wave_blocks = [
            m for m in decoder.modules() if isinstance(m, WaveReconstructionBlock)
        ]
        assert len(wave_blocks) >= 3
